...
import { registerUser, setStoredUser } from "@/lib/api";
...
export default function Signup() {
  ...
  const passwordStrength = (p: string) => {
    ...
    if (p.length < 7) return { label: "Fair", color: "bg-amber-400", width: "w-2/4" };
    if (p.length < 10) return { label: "Good", color: "bg-blue-400", width: "w-3/4" };
    return { label: "Strong", color: "bg-[#00b964]", width: "w-full" };
  };
      ...
      >
        ...
        <div className="px-8 py-7">
          ...
          <div className="text-center mb-5">
            ...
            <p className="text-gray-400 text-sm">Join thousands making a difference</p>
          </div>

          {/* Role toggle */}
          <div className="grid grid-cols-2 gap-2 mb-5 p-1 bg-gray-100 rounded-2xl">
            <button type="button" id="role-donor" onClick={() => setRole("donor")}
              className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ${role === "donor" ? "bg-white text-[#00b964] shadow-md" : "text-gray-500 hover:text-gray-700"}`}>
              <User size={15} /> Donor
            </button>
            <button type="button" id="role-organizer" onClick={() => setRole("organizer")}
              className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ${role === "organizer" ? "bg-white text-[#00b964] shadow-md" : "text-gray-500 hover:text-gray-700"}`}>
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-11 py-3 rounded-2xl border-2 bg-gray-50 focus:bg-white focus:outline-none text-sm transition-all duration-200 ${
                  confirm.length > 0 && confirm !== password
                    ? "border-red-300 focus:border-red-400"
                    : confirm.length > 0 && confirm === password
                    ? "border-[#00b964] bg-green-50"
                    : "border-gray-100 focus:border-[#00b964]"
                }`}
              />
              ...
              {confirm.length > 0 && confirm === password && (
                <span className="absolute right-10 top-1/2 -translate-y-1/2 text-[#00b964] text-xs">✓</span>
              )}
            </div>

            {/* Terms */}
            <label htmlFor="agree-checkbox" className="flex items-start gap-3 cursor-pointer select-none">
              <div className="relative mt-0.5 shrink-0">
                <input
                  ...
                  className="sr-only"
                />
                <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-200 ${agree ? "bg-[#00b964] border-[#00b964]" : "border-gray-300 bg-white"}`}>
                  {agree && <span className="text-white text-xs font-bold leading-none">✓</span>}
                </div>
              </div>
              <span className="text-xs text-gray-500 leading-relaxed">
                I agree to the{" "}
                <span className="text-[#00b964] font-semibold hover:underline">Terms & Conditions</span>
                {" "}and{" "}
                <span className="text-[#00b964] font-semibold hover:underline">Privacy Policy</span>
              </span>
            </label>
          ...
          <p className="text-center text-sm text-gray-500 mt-5">
            ...
            <Link href="/login" className="font-bold text-[#00b964] hover:underline">Sign in</Link>
          </p>
